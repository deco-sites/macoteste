...
function BannerItem({ image, lcp,sizes }: { image: Banner; lcp?: boolean,sizes:Sizes | undefined }) {
    const { alt, mobile, desktop, action, } = image;

    return (
        <a href={action?.href ?? "#"} aria-label={action?.label} class="relative overflow-y-hidden w-full" >
            <Picture preload={lcp}>
                <Source
...
function BannerCarousel({ images, preload, interval,sizes }: Props) {
    const id = useId();
            ...
            <Slider class="carousel carousel-center w-full col-span-full row-span-full scrollbar-none gap-6">
                {images?.map((image, index) => (
                    <Slider.Item index={index} class="carousel-item w-full">
                        <BannerItem image={image} sizes={sizes} lcp={index === 0 && preload} />
                    </Slider.Item>
                ))}
            </Slider>
